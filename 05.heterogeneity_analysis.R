...
calc_ATE <- function(data, drugs, pred.variable = "no_weight", weight.variable = NULL, breakdown = NULL, matching = FALSE, ntiles = 10, caliper = 0.05, replace = FALSE, order = "random", n_bootstrap = 100) {
  ...
  for (comb_i in 1:n_comb) {
    ...
    for (i in mnumber) {
      ...
      # collect mean
      obs <- append(obs, mean(bootstrap_differences))
...
ATE.var_adj.no_adj <- calc_ATE(cprd_dataset.dev, drugs = c("MFN", "DPP4", "GLP1", "SGLT2", "SU", "TZD"), pred.variable = "no_weight", n_bootstrap = 10, breakdown = breakdown)

# ATE adjusted for variables

ATE.var_adj.var_adj <- calc_ATE(cprd_dataset.dev, drugs = c("MFN", "DPP4", "GLP1", "SGLT2", "SU", "TZD"), pred.variable = "no_weight", breakdown = breakdown, n_bootstrap = 10)

# ATE overlap matching

ATE.var_adj.overlap_match <- calc_ATE(cprd_dataset.dev, drugs = c("MFN", "DPP4", "GLP1", "SGLT2", "SU", "TZD"), pred.variable = "no_weight", weight.variable = "overlap", matching = TRUE, n_bootstrap = 10)

# ATE IPW matching

ATE.var_adj.IPW_match <- calc_ATE(cprd_dataset.dev, drugs = c("MFN", "DPP4", "GLP1", "SGLT2", "SU", "TZD"), pred.variable = "no_weight", weight.variable = "IPW", matching = TRUE, n_bootstrap = 10)
