...
coefficients <- data.frame(
  cbind(
    variable = rep("Diabetes duration", 7),
    drug = drugs,
  ...
  rbind(
    ...
    data.frame(
      cbind(
        variable = rep("Total Cholesterol", 7),
        drug = drugs,
    ...
    data.frame(
      cbind(
        variable = rep("CKD (ref Stage 1)", 7),
        drug = drugs,
...
coefficients %>%
  ...
  geom_errorbar(aes(x = coef, xmin = LCI, xmax = UCI, y = drug, colour = drug)) +
  geom_pointrange(aes(x = coef, xmin = LCI, xmax = UCI, y = drug, colour = drug)) +
